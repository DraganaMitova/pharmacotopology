...
CERT = REPO_ROOT / "first_contact_clean_pharmacotopology_layer_run" / "V46_CFTR_F508DEL_MEMBRANE_MULTIDOMAIN_FOLDING_RESCUE_ATTACK" / "v46_cftr_f508del_membrane_multidomain_attack_certificate.json"
...
def _v46_committed() -> bool:
    return _git(["status", "--short", "--", *V46_PATHS]) == "" and CERT.exists()


def main() -> int:
    if not CERT.exists():
        raise SystemExit(f"missing V46 certificate: {CERT}")
    cert = json.loads(CERT.read_text(encoding="utf-8"))
    print("=== V46 CFTR F508DEL MEMBRANE MULTIDOMAIN ATTACK ===")
    print(f"V46 status: {cert.get('control_status')}")
    print(f"live_membrane_solution_packet: {cert.get('live_membrane_solution_packet')}")
    ...
    print(f"interdomain coupling support rate: {cert.get('interdomain_coupling_support_rate'):.3f}")
    print(f"rescue logic support rate: {cert.get('rescue_logic_support_rate'):.3f}")
    print("")
    ...
    print(f"V46 committed: {_v46_committed()}")
    print("")
