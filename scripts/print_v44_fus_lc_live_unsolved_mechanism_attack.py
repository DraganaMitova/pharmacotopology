...
CERT = REPO_ROOT / "first_contact_clean_pharmacotopology_layer_run" / "V44_FUS_LC_LIVE_UNSOLVED_MECHANISM_ATTACK" / "v44_fus_lc_live_unsolved_mechanism_attack_certificate.json"

V44_PATHS = [
    "scripts/build_v44_fus_lc_live_unsolved_sources_v0.py",
    "scripts/run_v44_fus_lc_live_unsolved_mechanism_attack_v0.py",
    "scripts/print_v44_fus_lc_live_unsolved_mechanism_attack.py",
    "tests/test_v44_fus_lc_live_unsolved_mechanism_attack.py",
    "docs/V44_FUS_LC_LIVE_UNSOLVED_MECHANISM_ATTACK_PROTOCOL.md",
    "data/live_unsolved_targets/V44/FUS_LC",
    "first_contact_clean_pharmacotopology_layer_run/V44_FUS_LC_LIVE_UNSOLVED_MECHANISM_ATTACK",
]
...
def _v43_commit_hash() -> str:
    return _git(["log", "--grep", "Add V43 protein folding solved-candidate trial", "-n", "1", "--format=%H"])


def _v44_committed() -> bool:
    return _git(["status", "--short", "--", *V44_PATHS]) == "" and CERT.exists()


def main() -> int:
    if not CERT.exists():
        raise SystemExit(f"missing V44 certificate: {CERT}")
    cert = json.loads(CERT.read_text(encoding="utf-8"))
    v43_hash = _v43_commit_hash()
    print("=== V44 FUS-LC LIVE UNSOLVED MECHANISM ATTACK ===")
    print(f"V43 committed: {bool(v43_hash)}")
    print(f"V43 commit hash: {v43_hash}")
    print(f"V44 status: {cert.get('control_status')}")
    ...
    print(f"V44 committed: {_v44_committed()}")
    print("")
