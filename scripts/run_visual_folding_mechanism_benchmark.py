...
from pharmacotopology.folding_visual_mechanism_benchmark import (  # noqa: E402
    run_visual_mechanism_benchmark,
)
...
DEFAULT_ROWS_PATH = Path(
    "first_contact_clean_pharmacotopology_layer_run/visual_mechanism_12_rows.csv"
)
DEFAULT_CONTACT_METRICS_PATH = Path(
    "first_contact_clean_pharmacotopology_layer_run/visual_mechanism_12_contact_metrics.csv"
)
DEFAULT_FAILURE_COHORTS_PATH = Path(
    "first_contact_clean_pharmacotopology_layer_run/visual_mechanism_12_failure_cohorts.csv"
)
DEFAULT_DASHBOARD_PATH = Path(
    "first_contact_clean_pharmacotopology_layer_run/visual_mechanism_12_dashboard.html"
)
DEFAULT_CERTIFICATE_PATH = Path(
    "first_contact_clean_pharmacotopology_layer_run/visual_mechanism_12_certificate.json"
)
DEFAULT_VISUALS_ROOT = Path(
    "first_contact_clean_pharmacotopology_layer_run/visuals"
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Render the locked 12-row visual folding mechanism workbench. "
            "This creates coarse contact maps, trajectories, energy curves, "
            "and safety artifacts without exposing raw sequences."
        )
    )
    ...
    outputs = run_visual_mechanism_benchmark(
