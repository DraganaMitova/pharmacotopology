#!/usr/bin/env python3
"""Export small locked runtime certificates into data/locked_runtime_certificates.

This prevents losing milestone evidence when the generated runtime directory is
ignored by git or replaced during repo updates. It copies JSON certificates only;
it never copies trajectories or heavy simulation artifacts.
"""
...
OUT = ROOT / "data" / "locked_runtime_certificates"

# Small critical milestone certificates. Add more here only if they are JSON lock/readout certificates.
CERTS = [
    ...
    ("V16_TRANSFER_DATA_PREFLIGHT", "v16_transfer_data_preflight_certificate.json"),
    ("V16_ZERO_MD_ROLE_TRANSFER_READOUT", "v16_zero_md_role_transfer_readout_certificate.json"),
    ("V16_PRESSURE_EVIDENCE_GAP_LOCK", "v16_pressure_evidence_gap_lock_certificate.json"),
    ("V17_PRESSURE_EVIDENCE_SPRINT_LOCK", "v17_pressure_evidence_sprint_lock_certificate.json"),
    ("V18_P53_TAD_MDM2_PARTNER_INDUCED_EVIDENCE_TEST", "v18_p53_tad_mdm2_partner_induced_evidence_certificate.json"),
    ...
    ("V19_KcsA_MEMBRANE_PORE_EVIDENCE_READOUT", "v19_kcsa_membrane_pore_evidence_readout_certificate.json"),
    ...
    ("V32_EXTERNAL_CONSTRAINT_SOURCE_IMPORT_PREFLIGHT", "v32_external_constraint_source_import_preflight_certificate.json"),
]
...
def main() -> None:
    ...
    index = {
        "kind": "LOCKED_RUNTIME_CERTIFICATE_EXPORT_INDEX_v0",
        "claim_allowed": False,
        "source_runtime_root": str(RUN_ROOT),
        "export_root": str(OUT),
    ...
    (OUT / "locked_runtime_certificates_index.json").write_text(json.dumps(index, indent=2, sort_keys=True), encoding="utf-8")
