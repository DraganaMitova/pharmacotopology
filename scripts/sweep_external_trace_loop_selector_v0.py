...
def _context_by_name(
    *,
    ...
    external_context = build_coupling_nucleus_context(
        rows=rows,
        coupling_dataset=external_dataset,
    )
    ...
    datasets.update(
        {name: control.dataset for name, control in adversarial_controls.items()}
    )
    contexts = {
    ...
    contexts["external_real"] = external_context
    return contexts
...
def run_selector_sweep(
    *,
    ...
    for index, config in enumerate(configs, start=1):
        ...
        adversarial_metrics = {}
        for control_name in EXTERNAL_ADVERSARIAL_CALIBRATED_CONTROL_NAMES:
            control_context = contexts[control_name]
            control_selected = select_sweep_trace_loop_events(control_context, config)
            adversarial_metrics[control_name] = selector_metrics(
                control_context,
                selector_name=f"{config.selector_config}_{control_name}",
                selected_events=control_selected,
            )
        ...
        adversarial_enrichments = [
            metric.real_vs_decoy_coupling_enrichment_ratio
            for metric in adversarial_metrics.values()
            if metric.selected_event_count
            >= ADVERSARIAL_ENRICHMENT_MIN_SELECTED_EVENTS
        ]
        max_adversarial_enrichment = (
            max(adversarial_enrichments) if adversarial_enrichments else 0.0
        )
        adversarial_enrichment_ratio = (
            _rounded(
                external_metric.real_vs_decoy_coupling_enrichment_ratio
                / max_adversarial_enrichment
            )
            if external_metric.selected_event_count > 0
            and max_adversarial_enrichment
            else 0.0
        )
        beats_physical = (
        ...
        beats_adversarial_calibrated_controls = (
            external_metric.selected_event_count > 0
            and all(
                metric.selected_event_count == 0
                or external_metric.false_nucleus_rate < metric.false_nucleus_rate
                for metric in adversarial_metrics.values()
            )
            and all(
                metric.selected_event_count == 0
                or external_metric.contact_cluster_precision
                > metric.contact_cluster_precision
                for metric in adversarial_metrics.values()
            )
            and all(
                metric.selected_event_count == 0
                or external_metric.long_range_contact_recall
        ...
        passes_adversarial_enrichment_gate = adversarial_enrichment_ratio > 1.0
        ...
        row = {
            **asdict(config),
            ...
            "external_vs_max_adversarial_calibrated_enrichment_ratio": (
                adversarial_enrichment_ratio
            ),
            ...
            "passes_adversarial_enrichment_gate": (
                passes_adversarial_enrichment_gate
            ),
            "external_probe_passed": external_probe_passed,
            "hard_adversarial_calibrated_probe_passed": (
                external_probe_passed
                and beats_adversarial_calibrated_controls
                and passes_adversarial_enrichment_gate
            ),
        }
        ...
        for control_name, metric in adversarial_metrics.items():
            row[_control_field("adversarial_selected", control_name)] = (
                metric.selected_event_count
            )
            row[_control_field("adversarial_false", control_name)] = (
                metric.false_nucleus_rate
            )
            row[_control_field("adversarial_precision", control_name)] = (
                metric.contact_cluster_precision
            )
            row[_control_field("adversarial_recall", control_name)] = (
                metric.long_range_contact_recall
            )
            row[_control_field("adversarial_enrichment", control_name)] = (
                metric.real_vs_decoy_coupling_enrichment_ratio
            )
        rows_out.append(row)
    ...
    rows_out.sort(
        key=lambda row: (
            not bool(row["hard_adversarial_calibrated_probe_passed"]),
            not bool(row["external_probe_passed"]),
            not bool(row["beats_adversarial_calibrated_controls"]),
            ...
            not bool(row["passes_adversarial_enrichment_gate"]),
            float(row["external_false_nucleus_rate"]),
            -float(row["external_cluster_precision"]),
            -float(row["external_vs_max_adversarial_calibrated_enrichment_ratio"]),
    ...
    for row in rows_out[:10]:
        print(
            ...
            "hard_adv=",
            row["hard_adversarial_calibrated_probe_passed"],
            "enrichment_vs_adv=",
            row["external_vs_max_adversarial_calibrated_enrichment_ratio"],
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and hard calibrated negative controls. This is exploratory and "
            "does not unlock claims."
        )
    )
