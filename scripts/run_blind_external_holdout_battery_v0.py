...
class ScaffoldContactMetrics:
    ...
    phase_coverage_scaffold_contact_count: int
    phase_coverage_scaffold_exact_contact_precision: float
    phase_coverage_scaffold_exact_long_range_contact_recall: float
    phase_coverage_scaffold_precision_recall_f1: float
    phase_coverage_scaffold_precision_delta_vs_phase_ribbon_bridge: float
    phase_coverage_scaffold_recall_delta_vs_phase_ribbon_bridge: float
    ...
    region_density_top_l_scaffold_contact_count: int
    region_density_top_l_scaffold_exact_contact_precision: float
    region_density_top_l_scaffold_exact_long_range_contact_recall: float
    region_density_top_l_scaffold_precision_recall_f1: float
    region_density_top_l_scaffold_precision_delta_vs_phase_coverage: float
    region_density_top_l_scaffold_recall_delta_vs_phase_coverage: float
    region_density_top_l_scaffold_contact_map_perfect: bool
    region_density_boundary_scaffold_contact_count: int
    region_density_boundary_scaffold_exact_contact_precision: float
    region_density_boundary_scaffold_exact_long_range_contact_recall: float
    region_density_boundary_scaffold_precision_recall_f1: float
    ...
    phase_density_conflict_consensus_scaffold_contact_count: int
    phase_density_conflict_consensus_scaffold_exact_contact_precision: float
    phase_density_conflict_consensus_scaffold_exact_long_range_contact_recall: float
    phase_density_conflict_consensus_scaffold_precision_recall_f1: float
...
def _valid_long_range_pairs(
    *,
    ...
    return {
        pair
        for pair in pairs
        if pair[0] >= 1 and pair[1] <= row_length and pair[1] - pair[0] >= 24
    }


def _diagonal_extend_pairs(
    *,
    row_length: int,
    pairs: set[tuple[int, int]],
    span: int,
    ...
    return _valid_long_range_pairs(row_length=row_length, pairs=expanded)


def _square_extend_pairs(
    *,
    row_length: int,
    pairs: set[tuple[int, int]],
    span: int,
...
def _phase_coverage_scaffold_core(
    *,
    row_length: int,
    phase_pairs: set[tuple[int, int]],
    phase_mode: str,
    ...
    if phase_mode == "square":
        return (
            _square_extend_pairs(row_length=row_length, pairs=phase_pairs, span=1),
            "square_one_cell",
            1,
        )
...
def _region_density_scores(
    *,
    ...
    for constraint in constraints:
        ...
        confidence_by_pair[pair] = max(
            confidence_by_pair.get(pair, 0.0),
            float(constraint.confidence),
        )
    scores: dict[tuple[int, int], float] = {}
    for pair in region_pairs:
        density = 0.0
        for constraint_pair, confidence in confidence_by_pair.items():
            distance = _contact_map_distance(pair, constraint_pair)
            if distance <= 3:
                density += confidence * (4 - distance) / 4
        scores[pair] = density
    return scores
...
def _phase_density_conflict_consensus_scaffold_core(
    *,
    phase_pairs: set[tuple[int, int]],
    density_pairs: set[tuple[int, int]],
    spine_pairs: set[tuple[int, int]],
) -> set[tuple[int, int]]:
    candidate_pairs = phase_pairs | density_pairs | spine_pairs
    return {
        pair
        for pair in candidate_pairs
        if (
            int(pair in phase_pairs)
            + int(pair in density_pairs)
            + int(pair in spine_pairs)
        )
        >= 2
    }
...
def _top_l_region_density_scaffold_core(
    *,
    ...
    ordered = sorted(
        density_scores.items(),
        key=lambda item: (item[1], item[0]),
        reverse=True,
    )
    return {pair for pair, _ in ordered[:row_length]}


def _boundary_region_density_scaffold_core(
    *,
    region_pairs: set[tuple[int, int]],
    constraints: Sequence[object],
    minimum_count: int,
...
def _precision_recall_f1(precision: float, recall: float) -> float:
    if precision + recall == 0.0:
        return 0.0
    return _rounded(2 * precision * recall / (precision + recall))
...
def _scaffold_contact_metrics(
    *,
    ...
    phase_coverage_precisions: list[float] = []
    phase_coverage_long_recalls: list[float] = []
    phase_coverage_f1s: list[float] = []
    phase_coverage_contact_counts: list[int] = []
    ...
    region_density_top_l_precisions: list[float] = []
    region_density_top_l_long_recalls: list[float] = []
    region_density_top_l_f1s: list[float] = []
    region_density_top_l_contact_counts: list[int] = []
    region_density_top_l_perfect_flags: list[bool] = []
    region_density_boundary_precisions: list[float] = []
    region_density_boundary_long_recalls: list[float] = []
    ...
    phase_density_conflict_consensus_precisions: list[float] = []
    phase_density_conflict_consensus_long_recalls: list[float] = []
    ...
    for row in rows:
        ...
        region_density_top_l_supported = (
            region_density_top_l_pairs & native_pairs
        )
        region_density_top_l_supported_long = (
            region_density_top_l_pairs & native_long
        )
        region_density_boundary_supported = (
            region_density_boundary_pairs & native_pairs
        )
        ...
        phase_density_conflict_consensus_supported = (
            phase_density_conflict_consensus_pairs & native_pairs
        )
        ...
        phase_coverage_precision = (
            _rounded(
                len(phase_coverage_supported) / len(phase_coverage_pairs)
            )
            if phase_coverage_pairs
            else 0.0
        )
        phase_coverage_long_recall = (
            _rounded(len(phase_coverage_supported_long) / len(native_long))
            if native_long
            else 1.0
        )
        phase_coverage_f1 = _precision_recall_f1(
            phase_coverage_precision,
            phase_coverage_long_recall,
        )
        phase_coverage_precisions.append(phase_coverage_precision)
        phase_coverage_long_recalls.append(phase_coverage_long_recall)
        phase_coverage_f1s.append(phase_coverage_f1)
        phase_coverage_contact_counts.append(len(phase_coverage_pairs))
        phase_coverage_perfect_flags.append(
            phase_coverage_precision == 1.0
            and phase_coverage_long_recall == 1.0
        )
        ...
        region_density_top_l_precision = (
            _rounded(
                len(region_density_top_l_supported)
                / len(region_density_top_l_pairs)
            )
            if region_density_top_l_pairs
            else 0.0
        )
        region_density_top_l_long_recall = (
            _rounded(
                len(region_density_top_l_supported_long) / len(native_long)
            )
            if native_long
            else 1.0
        )
        region_density_top_l_f1 = _precision_recall_f1(
            region_density_top_l_precision,
            region_density_top_l_long_recall,
        )
        region_density_top_l_precisions.append(region_density_top_l_precision)
        region_density_top_l_long_recalls.append(
            region_density_top_l_long_recall
        )
        region_density_top_l_f1s.append(region_density_top_l_f1)
        region_density_top_l_contact_counts.append(
            len(region_density_top_l_pairs)
        )
        region_density_top_l_perfect_flags.append(
            region_density_top_l_precision == 1.0
            and region_density_top_l_long_recall == 1.0
        )
        region_density_boundary_precision = (
            _rounded(
                len(region_density_boundary_supported)
                / len(region_density_boundary_pairs)
            )
            if region_density_boundary_pairs
            else 0.0
        )
        region_density_boundary_long_recall = (
            _rounded(
                len(region_density_boundary_supported_long) / len(native_long)
            )
            if native_long
            else 1.0
        )
        region_density_boundary_f1 = _precision_recall_f1(
            region_density_boundary_precision,
            region_density_boundary_long_recall,
        )
        region_density_boundary_precisions.append(
            region_density_boundary_precision
        )
        region_density_boundary_long_recalls.append(
            region_density_boundary_long_recall
        )
        region_density_boundary_f1s.append(region_density_boundary_f1)
        region_density_boundary_contact_counts.append(
            len(region_density_boundary_pairs)
        )
        region_density_boundary_perfect_flags.append(
        ...
        phase_density_conflict_consensus_precision = (
            _rounded(
                len(phase_density_conflict_consensus_supported)
                / len(phase_density_conflict_consensus_pairs)
            )
            if phase_density_conflict_consensus_pairs
            else 0.0
        )
        phase_density_conflict_consensus_long_recall = (
            _rounded(
                len(phase_density_conflict_consensus_supported_long)
                / len(native_long)
            )
            if native_long
            else 1.0
        )
        phase_density_conflict_consensus_f1 = _precision_recall_f1(
            phase_density_conflict_consensus_precision,
            phase_density_conflict_consensus_long_recall,
        )
        phase_density_conflict_consensus_precisions.append(
            phase_density_conflict_consensus_precision
        )
        phase_density_conflict_consensus_long_recalls.append(
            phase_density_conflict_consensus_long_recall
        )
        phase_density_conflict_consensus_f1s.append(
            phase_density_conflict_consensus_f1
        )
        phase_density_conflict_consensus_contact_counts.append(
            len(phase_density_conflict_consensus_pairs)
        )
        phase_density_conflict_consensus_perfect_flags.append(
    ...
    phase_coverage_precision = (
        _rounded(mean(phase_coverage_precisions))
        if phase_coverage_precisions
        else 0.0
    )
    phase_coverage_long_recall = (
        _rounded(mean(phase_coverage_long_recalls))
        if phase_coverage_long_recalls
        else 0.0
    )
    phase_coverage_f1 = (
    ...
    region_density_top_l_precision = (
        _rounded(mean(region_density_top_l_precisions))
        if region_density_top_l_precisions
        else 0.0
    )
    region_density_top_l_long_recall = (
        _rounded(mean(region_density_top_l_long_recalls))
        if region_density_top_l_long_recalls
        else 0.0
    )
    region_density_top_l_f1 = (
        _rounded(mean(region_density_top_l_f1s))
        if region_density_top_l_f1s
        else 0.0
    )
    region_density_boundary_precision = (
        _rounded(mean(region_density_boundary_precisions))
        if region_density_boundary_precisions
        else 0.0
    )
    region_density_boundary_long_recall = (
        _rounded(mean(region_density_boundary_long_recalls))
        if region_density_boundary_long_recalls
        else 0.0
    )
    ...
    phase_density_conflict_consensus_precision = (
        _rounded(mean(phase_density_conflict_consensus_precisions))
        if phase_density_conflict_consensus_precisions
        else 0.0
    )
    phase_density_conflict_consensus_long_recall = (
        _rounded(mean(phase_density_conflict_consensus_long_recalls))
        if phase_density_conflict_consensus_long_recalls
        else 0.0
    )
    phase_density_conflict_consensus_f1 = (
    ...
    return ScaffoldContactMetrics(
        ...
        phase_coverage_scaffold_contact_count=sum(
            phase_coverage_contact_counts
        ),
        phase_coverage_scaffold_exact_contact_precision=(
            phase_coverage_precision
        ),
        phase_coverage_scaffold_exact_long_range_contact_recall=(
            phase_coverage_long_recall
        ),
        phase_coverage_scaffold_precision_recall_f1=phase_coverage_f1,
        phase_coverage_scaffold_precision_delta_vs_phase_ribbon_bridge=(
            _rounded(
                phase_coverage_precision - phase_ribbon_bridge_precision
            )
        ),
        phase_coverage_scaffold_recall_delta_vs_phase_ribbon_bridge=(
            _rounded(
                phase_coverage_long_recall - phase_ribbon_bridge_long_recall
            )
        ),
        phase_coverage_scaffold_contact_map_perfect=(
            bool(phase_coverage_perfect_flags)
            and all(phase_coverage_perfect_flags)
        ),
        phase_coverage_scaffold_mode=";".join(phase_coverage_modes),
        phase_coverage_scaffold_span=(
            int(round(mean(phase_coverage_spans)))
        ...
        region_density_top_l_scaffold_contact_count=sum(
            region_density_top_l_contact_counts
        ),
        region_density_top_l_scaffold_exact_contact_precision=(
            region_density_top_l_precision
        ),
        ...
        region_density_top_l_scaffold_precision_delta_vs_phase_coverage=(
            _rounded(region_density_top_l_precision - phase_coverage_precision)
        ),
        region_density_top_l_scaffold_recall_delta_vs_phase_coverage=(
            _rounded(
                region_density_top_l_long_recall - phase_coverage_long_recall
            )
        ),
        region_density_top_l_scaffold_contact_map_perfect=(
            bool(region_density_top_l_perfect_flags)
            and all(region_density_top_l_perfect_flags)
        ),
        region_density_boundary_scaffold_contact_count=sum(
            region_density_boundary_contact_counts
        ),
        region_density_boundary_scaffold_exact_contact_precision=(
            region_density_boundary_precision
        ),
        region_density_boundary_scaffold_exact_long_range_contact_recall=(
            region_density_boundary_long_recall
        ),
        region_density_boundary_scaffold_precision_recall_f1=(
            region_density_boundary_f1
        ),
        region_density_boundary_scaffold_precision_delta_vs_phase_coverage=(
            _rounded(region_density_boundary_precision - phase_coverage_precision)
        ),
        region_density_boundary_scaffold_recall_delta_vs_phase_coverage=(
            _rounded(
                region_density_boundary_long_recall - phase_coverage_long_recall
            )
        ),
        ...
        phase_density_conflict_consensus_scaffold_contact_count=sum(
            phase_density_conflict_consensus_contact_counts
        ),
        phase_density_conflict_consensus_scaffold_exact_contact_precision=(
            phase_density_conflict_consensus_precision
        ),
        ...
        phase_density_conflict_consensus_scaffold_precision_delta_vs_phase_density_spine=(
            _rounded(
                phase_density_conflict_consensus_precision
                - phase_density_spine_precision
            )
        ),
        phase_density_conflict_consensus_scaffold_recall_delta_vs_phase_density_spine=(
            _rounded(
                phase_density_conflict_consensus_long_recall
                - phase_density_spine_long_recall
            )
...
def _folding_problem_solved_audit(
    *,
    ...
    requirements = {
        ...
        "all_targets_phase_coverage_scaffold_contact_maps_perfect": (
            bool(target_rows)
            and all(
                str(
                    row.get(
        ...
        "all_targets_region_density_top_l_scaffold_contact_maps_perfect": (
            bool(target_rows)
            and all(
                str(
                    row.get(
        ...
        "all_targets_region_density_boundary_scaffold_contact_maps_perfect": (
            bool(target_rows)
            and all(
                str(
                    row.get(
        ...
        "all_targets_phase_density_conflict_consensus_scaffold_contact_maps_perfect": (
            bool(target_rows)
            and all(
                str(
                    row.get(
...
def run_blind_external_holdout_battery_v0(
    *,
    ...
    report = {
        ...
        "mean_phase_coverage_scaffold_contact_count": _mean_field(
            target_rows,
            "phase_coverage_scaffold_contact_count",
        ),
        "mean_phase_coverage_scaffold_exact_contact_precision": _mean_field(
            target_rows,
            "phase_coverage_scaffold_exact_contact_precision",
        ),
        "mean_phase_coverage_scaffold_exact_long_range_contact_recall": (
            _mean_field(
                target_rows,
                "phase_coverage_scaffold_exact_long_range_contact_recall",
            )
        ...
        "mean_phase_coverage_scaffold_precision_delta_vs_phase_ribbon_bridge": (
            _mean_field(
                target_rows,
                (
                    "phase_coverage_scaffold_precision_delta_vs_"
                    "phase_ribbon_bridge"
                ),
            )
        ),
        "mean_phase_coverage_scaffold_recall_delta_vs_phase_ribbon_bridge": (
            _mean_field(
                target_rows,
                (
                    "phase_coverage_scaffold_recall_delta_vs_"
                    "phase_ribbon_bridge"
                ),
            )
        ),
        ...
        "mean_region_density_top_l_scaffold_contact_count": _mean_field(
            target_rows,
            "region_density_top_l_scaffold_contact_count",
        ),
        "mean_region_density_top_l_scaffold_exact_contact_precision": (
            _mean_field(
                target_rows,
                "region_density_top_l_scaffold_exact_contact_precision",
            )
        ),
        "mean_region_density_top_l_scaffold_exact_long_range_contact_recall": (
            _mean_field(
                target_rows,
                "region_density_top_l_scaffold_exact_long_range_contact_recall",
            )
        ),
        "mean_region_density_top_l_scaffold_precision_recall_f1": (
            _mean_field(
                target_rows,
                "region_density_top_l_scaffold_precision_recall_f1",
            )
        ),
        "mean_region_density_top_l_scaffold_precision_delta_vs_phase_coverage": (
            _mean_field(
                target_rows,
                (
                    "region_density_top_l_scaffold_precision_delta_vs_"
                    "phase_coverage"
                ),
            )
        ),
        "mean_region_density_top_l_scaffold_recall_delta_vs_phase_coverage": (
            _mean_field(
                target_rows,
                (
                    "region_density_top_l_scaffold_recall_delta_vs_"
                    "phase_coverage"
                ),
            )
        ),
        "region_density_top_l_scaffold_contact_map_perfect_rate": (
            _mean_field(
                target_rows,
                "region_density_top_l_scaffold_contact_map_perfect",
            )
        ),
        "mean_region_density_boundary_scaffold_contact_count": _mean_field(
            target_rows,
            "region_density_boundary_scaffold_contact_count",
        ),
        "mean_region_density_boundary_scaffold_exact_contact_precision": (
            _mean_field(
                target_rows,
                "region_density_boundary_scaffold_exact_contact_precision",
            )
        ),
        "mean_region_density_boundary_scaffold_exact_long_range_contact_recall": (
            _mean_field(
                target_rows,
                "region_density_boundary_scaffold_exact_long_range_contact_recall",
            )
        ),
        "mean_region_density_boundary_scaffold_precision_recall_f1": (
            _mean_field(
                target_rows,
                "region_density_boundary_scaffold_precision_recall_f1",
            )
        ),
        "mean_region_density_boundary_scaffold_precision_delta_vs_phase_coverage": (
            _mean_field(
                target_rows,
                (
                    "region_density_boundary_scaffold_precision_delta_vs_"
                    "phase_coverage"
                ),
            )
        ),
        "mean_region_density_boundary_scaffold_recall_delta_vs_phase_coverage": (
            _mean_field(
                target_rows,
                (
                    "region_density_boundary_scaffold_recall_delta_vs_"
                    "phase_coverage"
                ),
            )
        ),
        "region_density_boundary_scaffold_contact_map_perfect_rate": (
            _mean_field(
                target_rows,
                "region_density_boundary_scaffold_contact_map_perfect",
            )
        ...
        "mean_phase_density_conflict_consensus_scaffold_contact_count": (
            _mean_field(
                target_rows,
                "phase_density_conflict_consensus_scaffold_contact_count",
            )
        ),
        "mean_phase_density_conflict_consensus_scaffold_exact_contact_precision": (
            _mean_field(
                target_rows,
                (
                    "phase_density_conflict_consensus_scaffold_"
        ...
        "mean_phase_density_conflict_consensus_scaffold_exact_long_range_contact_recall": (
            _mean_field(
                target_rows,
                (
                    "phase_density_conflict_consensus_scaffold_"
        ...
        "mean_phase_density_conflict_consensus_scaffold_precision_recall_f1": (
            _mean_field(
                target_rows,
                (
                    "phase_density_conflict_consensus_scaffold_"
        ...
        "mean_phase_density_conflict_consensus_scaffold_precision_delta_vs_phase_density_spine": (
            _mean_field(
                target_rows,
                (
                    "phase_density_conflict_consensus_scaffold_"
                    "precision_delta_vs_phase_density_spine"
                ),
            )
        ),
        "mean_phase_density_conflict_consensus_scaffold_recall_delta_vs_phase_density_spine": (
            _mean_field(
                target_rows,
                (
                    "phase_density_conflict_consensus_scaffold_"
                    "recall_delta_vs_phase_density_spine"
                ),
            )
        ),
        "phase_density_conflict_consensus_scaffold_contact_map_perfect_rate": (
            _mean_field(
                target_rows,
                (
                    "phase_density_conflict_consensus_scaffold_"
