...
CERT = REPO_ROOT / "first_contact_clean_pharmacotopology_layer_run" / "V45_TDP43_LCD_LIVE_UNSOLVED_MECHANISM_ATTACK" / "v45_tdp43_lcd_live_unsolved_mechanism_attack_certificate.json"

V45_PATHS = [
    "scripts/build_v45_tdp43_lcd_live_unsolved_sources_v0.py",
    "scripts/run_v45_tdp43_lcd_live_unsolved_mechanism_attack_v0.py",
    "scripts/print_v45_tdp43_lcd_live_unsolved_mechanism_attack.py",
    "tests/test_v45_tdp43_lcd_live_unsolved_mechanism_attack.py",
    "docs/V45_TDP43_LCD_LIVE_UNSOLVED_MECHANISM_ATTACK_PROTOCOL.md",
    "data/live_unsolved_targets/V45/TDP43_LCD",
    "first_contact_clean_pharmacotopology_layer_run/V45_TDP43_LCD_LIVE_UNSOLVED_MECHANISM_ATTACK",
]
...
def _v44_commit_hash() -> str:
    return _git(["log", "--grep", "Add V44 FUS-LC live mechanism attack", "-n", "1", "--format=%H"])


def _v45_committed() -> bool:
    return _git(["status", "--short", "--", *V45_PATHS]) == "" and CERT.exists()


def main() -> int:
    if not CERT.exists():
        raise SystemExit(f"missing V45 certificate: {CERT}")
    cert = json.loads(CERT.read_text(encoding="utf-8"))
    v44_hash = _v44_commit_hash()
    print("=== V45 TDP-43 LCD LIVE UNSOLVED MECHANISM ATTACK ===")
    print(f"V44 committed: {bool(v44_hash)}")
    print(f"V44 commit hash: {v44_hash}")
    print(f"V45 status: {cert.get('control_status')}")
    print(f"live_unsolved_target_solution_packet: {cert.get('live_unsolved_target_solution_packet')}")
    ...
    for row in cert.get("validation_support_per_prediction", []):
        print(f"  - {row.get('prediction_id')}: {row.get('support_level')} - {row.get('perturbation')}")
    ...
    print(f"context shift support rate: {cert.get('context_shift_support_rate'):.3f}")
    print("")
    ...
    print(f"V45 committed: {_v45_committed()}")
    print("")
