...
class CouplingNucleusContext:
    ...
    def event_by_id(self) -> Mapping[str, NucleusClosureEvent]:
        # Candidate-frontier generation may promote events before they become
        # part of the older competitive set.  Event lookup must therefore see
        # the full candidate pool.  This does not change scoring inputs and does
        # not expose native/contact labels before selection.
        return {event.event_id: event for event in self.physical_context.candidate_events}
...
def _self_deciding_frontier_generation_score(
    ...
    *,
    ...
    This layer is intentionally broader than the older competitive-event
    expansion.  It can score any candidate region generated by the sequence
    closure layer, so it avoids the previous competitive-frontier ceiling.  It
    does not use native contacts, coordinates, accession names, or fixed
    confidence thresholds.  The downstream selector normalizes this score
    against the seed frontier's own row identity, then cuts at an internal gap.
    """
    region_pairs = set(event.candidate_region_pairs())
    if isinstance(row_constraints, Mapping):
        direct = tuple(
            row_constraints[pair] for pair in region_pairs if pair in row_constraints
        )
    else:
        direct = tuple(
    ...
    density_score = min(1.0, 32.0 * confidence_density)
    intrinsic_signal = max(
        0.0,
        0.28 * event.contact_cluster_gain
        + 0.16 * event.secondary_structure_compatibility
        + 0.16 * event.hydrophobic_burial_gain
        + 0.12 * event.registry_support
        + 0.14 * event.nucleus_score
        + 0.08 * event.closure_event_stability
    ...
    return {
        ...
        "candidate_generation_intrinsic_signal": _rounded(intrinsic_signal),
        "candidate_generation_density_score": _rounded(density_score),
        "native_truth_used_before_frontier_generation": False,
        "coordinate_truth_used_before_frontier_generation": False,
        "native_truth_used_before_frontier_expansion": False,
        "coordinate_truth_used_before_frontier_expansion": False,
    }


def _self_verified_generation_identity_baseline_rows(
    seed_events: Sequence[NucleusClosureEvent],
    context: CouplingNucleusContext,
    *,
    ...
    """Row-local identity baseline for broader candidate-frontier generation.

    The baseline is derived only from seed frontier events for the same row.
    Candidate-generator scores are compared against this local seed identity
    before the largest-gap cutoff is applied.
    """
    ...
    for row_id, events in seed_by_row.items():
        ...
        row_constraint_map = {constraint.pair(): constraint for constraint in row_constraints}
        ...
        for event in events:
            ...
            cheap_score = float(score_row["self_deciding_frontier_generation_score"])
            if cheap_score > 0.0:
        ...
        cheap_baseline = _median_positive(cheap_scores)
        cheap_lower_envelope = min((score for score in cheap_scores if score > 0.0), default=0.0)
        acceptance_baseline = _median_positive(acceptance_scores)
        acceptance_lower_envelope = min(
            (score for score in acceptance_scores if score > 0.0),
            default=0.0,
        )
        dominant_mode = mode_counts.most_common(1)[0][0] if mode_counts else "closed"
        output[row_id] = {
            "self_verified_generation_identity_baseline": acceptance_baseline,
            "self_verified_generation_identity_lower_envelope": _rounded(acceptance_lower_envelope),
            "self_verified_generation_identity_profile_baselines": profile_baseline,
            "self_verified_generation_cheap_baseline": cheap_baseline,
            "self_verified_generation_cheap_lower_envelope": _rounded(cheap_lower_envelope),
            "self_verified_generation_identity_dominant_mode": dominant_mode,
            "self_verified_generation_identity_seed_count": len(events),
            "native_truth_used_before_frontier_generation": False,
            "coordinate_truth_used_before_frontier_generation": False,
        }
    return output
...
def _generation_identity_normalized_score(
    *,
    row_id: str,
    profile: str,
    ...
    profile_baselines = baseline_row.get(
        "self_verified_generation_identity_profile_baselines",
        {},
    )
    profile_baseline = 0.0
    if isinstance(profile_baselines, Mapping):
        profile_baseline = float(profile_baselines.get(profile, 0.0))
    row_baseline = float(baseline_row.get("self_verified_generation_identity_baseline", 0.0))
    baseline = profile_baseline or row_baseline
    if baseline <= 0.0:
        return _rounded(acceptance_score), 0.0, "raw_generation_score_no_seed_identity_baseline"
    return _rounded(acceptance_score / baseline), _rounded(baseline), "frontier_generation_identity_normalized_seed_baseline"


def _candidate_generation_prefilter_cutoff(
    *,
    cheap_scores: Sequence[float],
    seed_cheap_lower_envelope: float | None,
) -> tuple[int, str, float]:
    """Choose which broad candidates deserve the expensive collapse audit.

    The prefilter is still self-deciding: it uses the candidate score
    distribution's largest gap plus the seed frontier's own row-local lower
    envelope.  This prevents collapse from evaluating thousands of impossible
    regions while avoiding a global score floor.
    """
    cutoff, reason, boundary = _self_deciding_frontier_expansion_cutoff(
        cheap_scores,
        seed_lower_envelope=seed_cheap_lower_envelope,
    )
...
def _events_by_identity_gap(
    scored_events: Sequence[tuple[float, NucleusClosureEvent, dict[str, object]]],
    *,
    seed_lower_envelope: float | None,
) -> tuple[tuple[NucleusClosureEvent, dict[str, object]], int, str, float]:
    ordered = sorted(
        scored_events,
        key=lambda item: (
            -item[0],
            -float(item[2].get("self_verified_frontier_generation_acceptance_score", 0.0)),
            -float(item[2].get("self_deciding_frontier_generation_score", 0.0)),
            -float(item[2].get("self_collapse_confidence", 0.0)),
            item[1].segment_a_start,
            item[1].segment_b_start,
            item[1].event_id,
        ),
    ...
    cutoff, reason, natural_boundary = _self_deciding_frontier_expansion_cutoff(
        [score for score, _event, _row in ordered],
        seed_lower_envelope=seed_lower_envelope,
    )
    selected = tuple((event, row) for _score, event, row in ordered[:cutoff])
    return selected, cutoff, reason, natural_boundary


def select_coupling_trace_loop_self_deciding_frontier_generated_events(
    context: CouplingNucleusContext,
    *,
    seed_events: Sequence[NucleusClosureEvent] = (),
) -> tuple[NucleusClosureEvent, ...]:
    """Promote frontier events from the broader candidate generator.

    This is the large frontier-generation probe: it can look beyond the older
    competitive-events ceiling.  It remains native-free: each row derives its
    identity baseline from seed frontier events, ranks candidate-generator
    regions by native-free evidence and collapse confidence, and cuts at the
    internal gap.  If the broad generation creates noisy regions, later audit
    reports mark the path as rejected instead of treating it as solved.
    """
    features_by_row, constraints_by_row = _collapse_inputs_for_context(context)
    collapse_cache: dict[str, dict[str, object]] = {}
    seed_by_row = _events_by_row(seed_events)
    identity_baseline_rows = _self_verified_generation_identity_baseline_rows(
        seed_events,
        context,
        features_by_row=features_by_row,
        constraints_by_row=constraints_by_row,
    ...
    for row in context.rows:
        ...
        if not expandable_profiles:
            ...
            continue
        baseline_row = identity_baseline_rows.get(row.row_id, {})
        cheap_baseline = float(baseline_row.get("self_verified_generation_cheap_baseline", 0.0))
        cheap_lower = float(baseline_row.get("self_verified_generation_cheap_lower_envelope", 0.0))
        cheap_lower_normalized = (
            _rounded(cheap_lower / cheap_baseline)
            if cheap_baseline > 0.0 and cheap_lower > 0.0
            else None
        )
        raw_candidates: list[tuple[float, NucleusClosureEvent, dict[str, object]]] = []
        ...
        for event in candidate_by_row.get(row.row_id, ()):
            ...
            cheap_score = float(score_row["self_deciding_frontier_generation_score"])
            normalized_cheap = (
                _rounded(cheap_score / cheap_baseline)
                if cheap_baseline > 0.0
                else cheap_score
            )
            raw_candidates.append((normalized_cheap, event, score_row))
        raw_candidates.sort(
            key=lambda item: (
                -item[0],
                -float(item[2]["self_deciding_frontier_generation_score"]),
                item[1].segment_a_start,
                item[1].segment_b_start,
                item[1].event_id,
            )
        )
        prefilter_cutoff, _prefilter_reason, _prefilter_boundary = (
            _candidate_generation_prefilter_cutoff(
                cheap_scores=[score for score, _event, _row in raw_candidates],
                seed_cheap_lower_envelope=cheap_lower_normalized,
            )
            if raw_candidates
            else (0, "empty_candidate_generation_distribution", 0.0)
        )
        prefiltered_candidates = raw_candidates[:prefilter_cutoff]
        scored_candidates: list[tuple[float, NucleusClosureEvent, dict[str, object]]] = []
        for _cheap_norm, event, score_row in prefiltered_candidates:
            collapse_row = _self_verified_collapse_row(
                event,
                context,
                features_by_row=features_by_row,
            ...
            normalized_score, identity_baseline, normalization_reason = (
                _generation_identity_normalized_score(
                    row_id=event.row_id,
                    profile=str(collapse_row["self_collapse_profile"]),
                    acceptance_score=acceptance_score,
                    baseline_rows=identity_baseline_rows,
            ...
            merged_row = {
                **score_row,
                **collapse_row,
                "self_verified_frontier_generation_acceptance_score": acceptance_score,
                "self_verified_frontier_generation_identity_normalized_score": normalized_score,
                "self_verified_frontier_generation_identity_baseline": identity_baseline,
                "self_verified_frontier_generation_normalization_reason": normalization_reason,
                "self_verified_frontier_generation_prefilter_cutoff_count": prefilter_cutoff,
                "self_verified_frontier_generation_prefilter_reason": _prefilter_reason,
                "self_verified_frontier_generation_prefilter_boundary": _prefilter_boundary,
                "native_truth_used_before_frontier_generation": False,
                "coordinate_truth_used_before_frontier_generation": False,
            }
            scored_candidates.append((normalized_score, event, merged_row))
        lower_envelope = float(baseline_row.get("self_verified_generation_identity_lower_envelope", 0.0))
        identity_baseline = float(baseline_row.get("self_verified_generation_identity_baseline", 0.0))
        normalized_lower_envelope = (
            _rounded(lower_envelope / identity_baseline)
            if identity_baseline > 0.0 and lower_envelope > 0.0
            else None
...
def self_deciding_frontier_generation_rows(
    context: CouplingNucleusContext,
    *,
    seed_events: Sequence[NucleusClosureEvent] = (),
    ...
    selected_ids = {
        event.event_id
        for event in select_coupling_trace_loop_self_deciding_frontier_generated_events(
            context,
    ...
    for row in context.rows:
        ...
        for event in candidate_by_row.get(row.row_id, ()):
            ...
            if int(score_row["direct_constraint_count"]) <= 0 and event.event_id not in selected_ids:
                continue
            normalized_cheap = (
                _rounded(float(score_row["self_deciding_frontier_generation_score"]) / cheap_baseline)
                if cheap_baseline > 0.0
                else float(score_row["self_deciding_frontier_generation_score"])
            )
            score_row["self_verified_frontier_generation_cheap_identity_normalized_score"] = normalized_cheap
            score_row["seed_event"] = event.event_id in seed_ids
            raw_rows.append(score_row)
        raw_rows.sort(
            key=lambda item: (
                -float(item["self_verified_frontier_generation_cheap_identity_normalized_score"]),
                -float(item["self_deciding_frontier_generation_score"]),
                str(item["event_id"]),
            ),
        )
        prefilter_cutoff, prefilter_reason, prefilter_boundary = _candidate_generation_prefilter_cutoff(
            cheap_scores=[float(item["self_verified_frontier_generation_cheap_identity_normalized_score"]) for item in raw_rows],
            seed_cheap_lower_envelope=cheap_lower_normalized,
        )
        prefilter_ids = {str(item["event_id"]) for item in raw_rows[:prefilter_cutoff]}
        candidate_rows: list[dict[str, object]] = []
        expandable_profiles = expandable_profiles_by_row.get(row.row_id, set())
        for raw_row in raw_rows:
            event = context.event_by_id[str(raw_row["event_id"])]
            event_id = str(raw_row["event_id"])
            # The selector above already performed the expensive collapse audit
            # needed to decide the generated frontier.  The report keeps raw
            # candidate-generator rows visible, but only recomputes collapse
            # details for seed/selected events.  This keeps the diagnostics
            # finite without introducing a new acceptance threshold.
            should_collapse = raw_row["seed_event"] is True or event_id in selected_ids
            collapse_row = (
                _self_verified_collapse_row(
                    event,
                    context,
                    features_by_row=features_by_row,
                    constraints_by_row=constraints_by_row,
                ...
                if should_collapse
                else _closed_self_verified_collapse_row()
            )
            candidate_profile_allowed = (
                raw_row["seed_event"] is False
                and event_id in selected_ids
                and str(collapse_row["self_collapse_profile"]) in expandable_profiles
                and int(collapse_row["self_collapse_selected_pair_count"]) > 0
            )
            acceptance_score = (
                _self_verified_frontier_expansion_acceptance_score(raw_row, collapse_row)
            ...
            normalized_score, identity_baseline, normalization_reason = (
                ...
                if candidate_profile_allowed
                else (0.0, 0.0, "not_candidate_for_frontier_generation")
            )
            merged = {
                **raw_row,
                **collapse_row,
                ...
                "self_verified_frontier_generation_identity_baseline": identity_baseline,
                "self_verified_frontier_generation_normalization_reason": normalization_reason,
                "self_verified_frontier_generation_prefilter_selected": str(raw_row["event_id"]) in prefilter_ids,
                "self_verified_frontier_generation_prefilter_cutoff_count": prefilter_cutoff,
                "self_verified_frontier_generation_prefilter_reason": prefilter_reason,
                "self_verified_frontier_generation_prefilter_boundary": prefilter_boundary,
                "self_deciding_frontier_generation_selected": str(raw_row["event_id"]) in selected_ids,
                "self_verified_frontier_generation_selected": str(raw_row["event_id"]) in selected_ids,
                "self_verified_frontier_generation_gate_reason": (
                    "seed_event"
                    if raw_row["seed_event"] is True
                    else "candidate_generator_identity_gap_selected"
                    if str(raw_row["event_id"]) in selected_ids
                    else "candidate_generator_not_selected"
                ),
                "native_truth_used_before_frontier_generation": False,
                "coordinate_truth_used_before_frontier_generation": False,
            }
        ...
        ordered_candidates = sorted(
            candidate_rows,
            key=lambda item: (
                -float(item["self_verified_frontier_generation_identity_normalized_score"]),
                -float(item["self_verified_frontier_generation_acceptance_score"]),
                -float(item["self_deciding_frontier_generation_score"]),
                -float(item["self_collapse_confidence"]),
                str(item["event_id"]),
            ),
        )
        lower_envelope = float(baseline_row.get("self_verified_generation_identity_lower_envelope", 0.0))
        ...
        cutoff, reason, natural_boundary = _self_deciding_frontier_expansion_cutoff(
            [float(item["self_verified_frontier_generation_identity_normalized_score"]) for item in ordered_candidates],
            seed_lower_envelope=normalized_lower_envelope,
        )
        rank_by_id = {str(item["event_id"]): rank for rank, item in enumerate(ordered_candidates, start=1)}
        for item in rows:
            if str(item.get("row_id")) != row.row_id:
                continue
            rank = rank_by_id.get(str(item.get("event_id")), 0)
            item["self_verified_frontier_generation_gap_rank"] = rank
            item["self_verified_frontier_generation_cutoff_count"] = cutoff
            item["self_verified_frontier_generation_cutoff_reason"] = reason
            item["self_verified_frontier_generation_natural_boundary"] = natural_boundary
            item["self_verified_frontier_generation_gap_selected"] = rank > 0 and rank <= cutoff
    return sorted(
        rows,
        key=lambda item: (
            str(item["row_id"]),
            -float(item.get("self_deciding_frontier_generation_score", 0.0)),
            str(item["event_id"]),
        ),
    )
