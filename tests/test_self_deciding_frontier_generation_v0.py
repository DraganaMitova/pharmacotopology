...
def test_4ake_candidate_pool_generation_is_audited_but_rejected_as_main() -> None:
    ...
    assert decision["frontier_generation_decision"] == "rejected_precision_gain_without_recall_gain"
    assert generated["collapsed_contact_precision"] >= 0.40
...
def test_4ake_generation_promotes_seed_identity_continuation_without_native_truth() -> None:
    report = _report()
    rows = report["self_deciding_frontier_generation_rows"]
    selected = [
        row
        ...
        and row["self_verified_frontier_generation_selected"] is True
    ]

    assert selected
    assert any(
        float(row["candidate_generation_seed_identity_continuation_score"]) > 0.80
        and row["self_collapse_tier_mode"] == "direct_ridge_trace_low_score_tier"
        for row in selected
    )
    assert all(row["native_truth_used_before_frontier_generation"] is False for row in selected)
    assert all(row["coordinate_truth_used_before_frontier_generation"] is False for row in selected)
    assert all("self_collapse_lattice_completeness" in row for row in selected)
    assert all("self_collapse_alpha_compactness" in row for row in selected)
